...
export class IPClass {
     ...
     * @returns 
     */
    public static async getIP4ArrayFromNetworkInterface(): Promise<Array<string>> {
    ...
    public static async getIp(matchRegexStr: string, filterRegexStr: string): Promise<string> {
        let ip = ``;
        ...
        for (const ipStr of IpArray) {
            let matchStr = `'${ipStr}'.match(${matchRegexStr})`;
            let filterStr = `'${ipStr}'.match(${filterRegexStr})`;
